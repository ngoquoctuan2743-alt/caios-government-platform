...
import { Card, CardContent, Badge, Tabs, TabsList, TabsTrigger, TabsPanel } from "@ds/components";

type Category = "case" | "system" | "ai";

const NOTIFICATIONS: { id: string; category: Category; icon: typeof Bell; tone: "success" | "warning" | "primary" | "info"; text: string; time: string; read: boolean }[] = [
  ...
  { id: "4", category: "system", icon: Server, tone: "info", text: "Hệ thống sẽ bảo trì định kỳ vào 02:00 ngày 20/07/2026.", time: "3 ngày trước", read: true },
    ...
    <div className="mx-auto max-w-[720px] space-y-6 px-6 py-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--vdg-color-text-secondary)]">{unreadCount} thông báo chưa đọc</p>
        </div>
        <Badge variant="primary">{NOTIFICATIONS.length} tổng cộng</Badge>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsPanel value={tab} className="mt-4 space-y-3">
          {filtered.length === 0 ? (
            <p className="py-8 text-center text-sm text-[var(--vdg-color-text-secondary)]">Không có thông báo nào.</p>
          ) : (
            filtered.map((n) => (
              <Card key={n.id} variant="solid" className={n.read ? "opacity-70" : undefined}>
                <CardContent className="flex items-start gap-3 py-4">
                  ...
                  {!n.read && <span className="mt-1.5 size-2 shrink-0 rounded-full bg-[var(--vdg-color-primary)]" />}
                </CardContent>
              </Card>
            ))
          )}
