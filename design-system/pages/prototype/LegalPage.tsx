...
import type { LegalDoc } from "./navigation";
...
export function LegalPage({ doc }: { doc: LegalDoc }) {
  const { title, updated, sections } = CONTENT[doc];
  return (
    <div className="mx-auto max-w-[720px] space-y-6 px-6 py-10">
      <div className="flex items-center gap-3">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-[var(--vdg-color-primary)]/10 text-[var(--vdg-color-primary)]">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--vdg-color-text-secondary)]">{updated}</p>
        </div>
      </div>
      <Card variant="solid">
        <CardContent className="space-y-6 py-8">
          {sections.map((s) => (
            <div key={s.heading}>
              ...
              <p className="text-sm leading-relaxed text-[var(--vdg-color-text-secondary)]">{s.body}</p>
            </div>
          ))}
        </CardContent>
      </Card>
