...
import { HeroBackground } from "@/components/government/assets/backgrounds"
import { AIAssistantIllustration, SmartCityIllustration } from "@/components/government/assets/illustrations"
import { useGovernment } from "@/components/government/providers"
import { GOV_ROUTES } from "@/components/government/routes"
import type { Locale } from "@/components/government/providers"
...
export function LandingPage() {
  ...
  function goSearch(q: string) {
    router.push(`${GOV_ROUTES.search}?q=${encodeURIComponent(q)}`)
  }
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-[1320px] px-6 pb-20 pt-20 sm:pt-28">
          <Badge className="mb-6 bg-accent text-accent-foreground">
            <Sparkles className="size-3" /> {c.aiBadge}
          </Badge>
          <h1 className="font-heading max-w-[18ch] text-[clamp(2.5rem,1.8rem+3vw,4.25rem)] font-bold leading-[1.05] tracking-tight">
            {c.heroTitle}{" "}
            <span className="bg-gradient-to-r from-primary to-[color-mix(in_oklch,var(--primary),black_25%)] bg-clip-text text-transparent">
              {c.heroTitleAccent}
            </span>
          </h1>
          <p className="mt-5 max-w-[52ch] text-lg text-muted-foreground">{c.heroSubtitle}</p>
          ...
          <form
            ...
            className="group mt-10 flex max-w-2xl flex-col gap-3 rounded-2xl border border-border bg-card p-2 shadow-lg transition-shadow duration-300 focus-within:border-primary/50 sm:flex-row sm:items-center"
          >
            <div className="flex flex-1 items-center gap-2.5 px-3 py-2">
              <Search className="size-4.5 shrink-0 text-muted-foreground transition-colors group-focus-within:text-primary" />
              <input
                type="search"
                value={query}
          ...
          <div className="mt-16 grid grid-cols-2 gap-6 border-t border-border pt-8 sm:grid-cols-4">
            {c.stats.map((s) => (
              <div key={s.label}>
                <div className="font-heading text-2xl font-bold tabular-nums sm:text-3xl">{s.value}</div>
      ...
      <section className="border-t border-border py-20">
        <div className="mx-auto max-w-[1320px] px-6">
          <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-primary">{c.servicesEyebrow}</span>
              <h2 className="font-heading text-2xl font-bold sm:text-3xl">{c.servicesTitle}</h2>
            </div>
            <button type="button" onClick={() => goSearch("")} className="flex items-center gap-1.5 text-sm font-medium text-primary">
              {c.servicesAll} <ArrowRight className="size-4" />
            </button>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {SERVICES.map(({ icon: ServiceIcon, label, desc }) => (
              <button key={label} type="button" onClick={() => goSearch(label)} className="text-left">
      ...
      <section className="border-t border-border py-20">
        <div className="mx-auto max-w-[1320px] px-6">
          <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-primary">{c.newsEyebrow}</span>
              <h2 className="font-heading text-2xl font-bold sm:text-3xl">{c.newsTitle}</h2>
            </div>
            <button type="button" onClick={() => goSearch("")} className="flex items-center gap-1.5 text-sm font-medium text-primary">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {NEWS.map(({ icon: NewsIcon, date, title, excerpt }) => (
              <button key={title} type="button" onClick={() => goSearch(title)} className="text-left">
                <Card className="h-full transition-shadow hover:shadow-md hover:ring-primary/30">
                  <CardContent>
      ...
      <section className="border-t border-border py-20">
        <div className="mx-auto grid max-w-[1320px] grid-cols-1 items-center gap-10 px-6 lg:grid-cols-2">
          <div>
            <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-primary">{c.aiEyebrow}</span>
            <h2 className="font-heading text-2xl font-bold sm:text-3xl">{c.aiTitle}</h2>
            <p className="mt-4 max-w-[48ch] text-muted-foreground">{c.aiSubtitle}</p>
            <Button variant="default" className="mt-6" nativeButton={false} render={<Link href={GOV_ROUTES.login} />}>
              {c.aiCta}
            </Button>
          </div>
      ...
      <section className="border-t border-border py-16">
        <div className="mx-auto grid max-w-[1320px] grid-cols-1 items-center gap-10 px-6 lg:grid-cols-2">
          ...
          <div className="lg:order-1">
            <h3 className="font-heading text-xl font-bold sm:text-2xl">{c.platformTitle}</h3>
            <p className="mt-3 max-w-[46ch] text-sm text-muted-foreground sm:text-base">{c.platformSubtitle}</p>
          </div>
        </div>
      </section>
    </div>
