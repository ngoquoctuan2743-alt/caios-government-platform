...
export default function CaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-6 py-8">
      <button type="button" onClick={() => router.back()} className="mb-6 flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" /> Quay lại
      </button>

      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl font-bold">{decodeURIComponent(id)}</h1>
          <p className="mt-1 text-muted-foreground">Đăng ký kinh doanh · Trần Thị B</p>
        </div>
        <StatusBadge tone="primary">Đang xử lý</StatusBadge>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ShieldCheck className="size-4 text-primary" /> Luồng phê duyệt
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <textarea
                ...
                className="w-full rounded-md border border-border bg-background p-3 text-sm outline-none focus-visible:border-primary"
              />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <FileText className="size-4 text-primary" /> Giấy tờ
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2.5">
              {DOCUMENTS.map((d) => (
                <div key={d.name} className="flex items-center justify-between rounded-md border border-border px-3 py-2.5 text-sm">
                  <span className="flex items-center gap-2.5">
                    <ListChecks className="size-4 text-muted-foreground" />
                    {d.name}
                  </span>
                  <StatusBadge tone={d.status}>{d.label}</StatusBadge>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Clock3 className="size-4 text-primary" /> Nhật ký hoạt động
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-6">
          <Card className="border-primary/20 bg-primary/[0.03]">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Sparkles className="size-4 text-primary" /> Tóm tắt AI
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Accordion>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <MessageSquareText className="size-4 text-muted-foreground" /> Liên hệ công dân
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <p className="text-sm text-muted-foreground">Gửi yêu cầu bổ sung hoặc câu hỏi trực tiếp tới công dân.</p>
